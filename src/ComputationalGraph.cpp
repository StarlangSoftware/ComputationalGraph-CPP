 ...
 */
void ComputationalGraph::clearRecursive(set<ComputationalNode *>& visited, ComputationalNode *node) {
    visited.emplace(node);
    if (!node->isLearnable()) {
        node->setValueNull();
    }
...
Tensor ComputationalGraph::calculateDerivative(ComputationalNode *node, ComputationalNode *child) {
    ...
    if (child->getFunction() != nullptr) {
        ...
        if (child->getNodeType() == CONCATENATED_NODE_TYPE) {
            ...
            int blockSize = backward.getShape()[_child->getDimension()] / reverseChildren.size();
            ...
            if (child->getNodeType() == MULTIPLICATION_NODE_TYPE) {
                ...
                if (left == node) {
                    ...
                    return backward.multiply(rightValue.transpose(transposeAxes(rightValue.getShape().size())));
                }
                ...
                if (!left->isValueNull()) {
                    return leftValue.transpose(transposeAxes(leftValue.getShape().size())).multiply(backward);
                }
            }
            return backward;
        }
...
ComputationalNode * ComputationalGraph::addEdge(ComputationalNode *first, Function *function, const bool isBiased) {
    auto* newNode = new ComputationalNode(false, function, isBiased);
    computeIfAbsent(nodeMap, first, newNode);
    computeIfAbsent(reverseNodeMap, newNode, first);
    return newNode;
}
...
ComputationalNode * ComputationalGraph::addAdditionEdge(ComputationalNode *first, ComputationalNode *second,
    const bool isBiased) {
 ...
 */
ComputationalNode * ComputationalGraph::concatEdges(const vector<ComputationalNode *>& nodes, const int dimension) {
...
void ComputationalGraph::backpropagation(Optimizer *optimizer, const vector<int>& classLabelIndex) {
 ...
 */
void ComputationalGraph::calculateRMinusY(ComputationalNode *output, const vector<int>& classLabelIndex) {
    vector<double> values;
    const vector<double> outputValues = output->getValue().getData();
    values.reserve(outputValues.size());
    int lastDimension = output->getValue().getShape()[output->getValue().getShape().size() - 1];
...
void ComputationalGraph::getBiased(ComputationalNode *tensor) {
    int lastDimensionSize = tensor->getValue().getShape()[tensor->getValue().getShape().size() - 1];
    vector<double> values;
                    ...
                    if (child->getFunction() != nullptr) {
                        ...
                        if (function->type != FunctionType::DROPOUT) {
                            if (enableDropout) {
                                ...
                                child->setValue(Tensor(currentValue.getData(), currentValue.getShape()));
                            }
                        } else {
                            child->setValue(function->calculate(currentValue));
                        }
                        ...
                        if (child->getNodeType() == CONCATENATED_NODE_TYPE) {
                            if (!concatenatedNodeMap.contains(child)) {
                                vector<ComputationalNode*> items;
                                items.reserve(reverseNodeMap[child].size());
                                concatenatedNodeMap[child] = vector<ComputationalNode*>();
                            }
                            concatenatedNodeMap[child][((ConcatenatedNode*)child)->getIndex(*currentNode)] = currentNode;
                            ...
                            if (reverseNodeMap[child].size() == counterMap[child]) {
                                ...
                                for (int i = 1; i < concatenatedNodeMap[child].size(); i++) {
                                    child->setValue(child->getValue().concat(concatenatedNodeMap[child][i]->getValue(), ((ConcatenatedNode*) child)->getDimension()));
                                }
                            }
                        } else {
