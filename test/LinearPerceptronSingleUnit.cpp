...
vector<int>
LinearPerceptronSingleUnit::getClassLabels(ComputationalNode *outputNode) {
  vector<int> labels;
  labels.push_back(0);
  return labels;
}

void LinearPerceptronSingleUnit::train(vector<Tensor> &trainSet,
  ...
  auto outputNode = addFunctionEdge(a, softmax, false);
...
LinearPerceptronSingleUnit::test(const vector<Tensor> &testSet) {
